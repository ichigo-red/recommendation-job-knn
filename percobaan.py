...
try:
    knn_model = joblib.load(open('knn_model.joblib','rb'))
    vectorizer = joblib.load(open('vectorizer.joblib', 'rb'))
    scaler = joblib.load(open('normalization_params.joblib', 'rb'))
...
menu = st.sidebar.radio("Menu", ["Rekomendasi Pekerjaan", "Tentang Aplikasi", "Tentang Kami"])
...
if menu == "Rekomendasi Pekerjaan":
    if data_old is not None and data_new is not None:
        ...
        major = st.selectbox("Jurusan :", data_old["UG Specialization (Major)"].unique())
